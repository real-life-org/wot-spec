...
**deviceId-Lifecycle (MUSS).** Die `deviceId` MUSS im selben durablen Speicher wie der Log dieses Devices liegen und denselben Lebenszyklus teilen: jeder lokale Ablauf, der den Log oder die lokalen Identitätsdaten eines Devices **destruktiv löscht** (z.B. Reset/Create-New, Identitäts-Löschung, Legacy-Break), MUSS `deviceId` und Log **gemeinsam** entfernen. Ein reiner Session-Logout ohne Log-Wipe ist davon nicht betroffen (der erhaltene Log bleibt nonce-sicher). Überlebt eine `deviceId` einen Log-Wipe (separate Persistenz), schreibt das Device danach ab `seq=0` unter der **alten** `deviceId` — die deterministische Nonce aus `(deviceId, seq)` (siehe [Sync 001](001-verschluesselung.md#nonce-konstruktion)) kollidiert dann mit bereits vergebenen Nonces = AES-256-GCM-Nonce-Reuse. Ein Wipe MUSS daher eine frische `deviceId` (frischer Nonce-Namespace) nach sich ziehen.

**Eine Quelle für Registrierung und Autorenschaft (MUSS).** Die `deviceId`, mit der ein Client beim Broker registriert (siehe [Sync 003 Authentisierung](003-transport-und-broker.md#authentisierung)), MUSS dieselbe sein, unter der er Log-Einträge signiert — eine einzige, **vor** dem Verbindungsaufbau aus dem durablen Speicher aufgelöste `deviceId`. Andernfalls ist die zum Schreiben verwendete `deviceId` für die registrierte DID nicht hinterlegt und die Broker-[Autor-Bindung](003-transport-und-broker.md#log-eintrag-autor-bindung-muss) verwirft die Einträge.
...
### Vollstaendigkeits-Cursor, Luecken und Pagination

Die `heads` eines `sync-request` (siehe [Sync 003](003-transport-und-broker.md#sync-request10--anfrage-was-hast-du-seit-x)) sind der Vollstaendigkeits-Cursor: sie sagen dem Antwortenden, ab welcher `seq` pro `deviceId` noch Eintraege fehlen. Daraus folgen vier normative Regeln:

- **Kontiger Cursor (MUSS).** Der `heads`-Wert pro `deviceId` MUSS die hoechste **lueckenlose** `seq` sein, nicht die hoechste ueberhaupt bekannte. Oberhalb einer Luecke empfangene Eintraege werden angewandt (der CRDT-Merge ist kommutativ und idempotent), ruecken den Cursor aber **nicht** ueber die Luecke. Ein einzelner Maximalwert kann eine Luecke nicht ausdruecken; als Cursor gefuehrt wuerde eine fehlende `seq` maskiert und nie nachgefordert (stiller Datenverlust). Die Maximal-`seq` pro `deviceId` bleibt nur fuer den `broker_seq > local_seq`-Restore/Clone-Vergleich relevant (siehe [seq-Konsistenz](#seq-konsistenz-muss)).
- **Pagination-Terminierung (MUSS).** Liefert eine `sync-response` `truncated: true`, MUSS der Client weitere `sync-request`-Runden senden, bis `truncated: false`. Die Heads der Folge-Anfrage MUSS der Client aus seinem **lokalen kontigen Cursor** nach Verarbeitung der verifizierten Eintraege berechnen — **nicht** durch Uebernahme von `sync-response.heads` (das ist das Maximum des Antwortenden, kein Cursor; siehe [Sync 003 sync-response](003-transport-und-broker.md#sync-response10--antwort-mit-fehlenden-einträgen)). Fortschritt wird am kontigen Cursor gemessen. Eine Seite, die den Cursor nicht vorrueckt, aber verifizierte Eintraege lieferte (auch bereits angewandte, idempotente), ist **kein** Abbruchgrund, sondern ein Luecken-Zustand. Nur eine `truncated: true`-Seite **ohne jede** verifizierte Eintrags-Evidenz ist ein harter Abbruch (defekter Antwortender).
- **Dauerhaft fehlende `seq` (MUSS).** Liefert der Antwortende auf einen `sync-request` als **niedrigste** `seq` fuer ein `deviceId` einen Wert **groesser** als die erste lokal fehlende `seq`, ist diese `seq` autoritativ abwesend — **auch** bei `truncated: true`. (Liefert er die kontig naechste `seq`, ist es nur eine Pagination-Grenze, keine dauerhafte Luecke.) Eine `seq` kann dauerhaft fehlen, etwa wenn ein Eintrag nie geschrieben oder durch ein spaeteres Protokoll-Ereignis ersetzt wurde.
- **Ueberspringen einer dauerhaft fehlenden `seq` (MUSS).** Der Cursor DARF eine Luecke nur ueberspringen, **nachdem** sie ueber **mehrere distinkte Verbindungs-Epochen** und ein Mindestalter hinweg als abwesend bestaetigt wurde (Schutz gegen transiente Luecken). Nach dem Ueberspringen MUSS eine durable Markierung bestehen bleiben, die den uebersprungenen Bereich weiter nachfordert; trifft ein Eintrag spaeter doch ein, konvergiert er (kein finaler, irreversibler Skip). Die konkreten Schwellen (Anzahl Epochen, Mindestalter) sind implementationsspezifisch, nicht normativ.
...
**`KEY_GENERATION_STALE`-Behandlung (MUSS):** Lehnt ein Broker einen `log-entry` mit `KEY_GENERATION_STALE` ab — die `keyGeneration` des Eintrags ist strikt kleiner als die aktuelle `space.generation` (Broker-Ingest-Generations-Gate, [Sync 003](003-transport-und-broker.md)) —, hat der Client eine Rotation verpasst. Er MUSS die fehlende `key-rotation`/Generation nachladen ([Key-Rotation und Generation-Gaps](#key-rotation-und-generation-gaps)) und den Schreibvorgang unter einer **neuen `seq`** (`reserveNext`) mit der neuen `keyGeneration` erneut ausführen. Er DARF den abgelehnten Eintrag **nicht** unter derselben `seq` mit der neuen Generation re-emittieren: Falls der Alt-Gen-Eintrag bei einem anderen (noch nicht rotierten) Home-Broker bereits persistiert wurde, würde derselbe `(deviceId, seq)` mit abweichendem Content-Hash `SEQ_COLLISION_DETECTED` auslösen und den legitimen Client fälschlich als `seq`-/Nonce-Reuse hart abbrechen lassen (siehe [seq-Konsistenz](#seq-konsistenz-muss)). Der Re-emit MUSS **denselben CRDT-Update** tragen wie der abgelehnte Eintrag; ein bereits persistierter Alt-Gen-Eintrag bleibt **gültige History** (er war ein legitimer Pre-Enforcement-Write, siehe [Sync 005 Removal-Enforcement-Semantik](005-gruppen.md#removal-enforcement-semantik-muss)) und wird weiter mit seiner historischen `keyGeneration` entschlüsselt — das doppelte Anwenden (Alt-Gen-Eintrag + Re-emit) ist idempotent, da Yjs/Automerge denselben Update intern deduplizieren. **Keine** generelle „Alt-Gen ignorieren"-Regel.
