...
2. Der Admin registriert die neue Capability-Key-Generation beim Broker (`space-rotate`) **und wartet die Bestätigung ab**, oder legt die Operation in eine retrybare Outbox, falls der Broker offline ist — die Outbox-Variante stellt das Removal in einen **`pending`-Zustand: noch NICHT durchgesetzt** (siehe [Removal-Enforcement-Semantik](#removal-enforcement-semantik-muss)).
...
Eine Member-Entfernung gilt erst als **durchgesetzt**, wenn der Broker die `space-rotate` **bestätigt** hat (neue `space.generation` aktiv). Daraus folgt:

- Das clientseitige `removeMember` DARF gegenüber App/User **nicht als durchgesetztes Removal** signalisieren, solange der Broker nicht rotiert hat. Online: Remove-Operation + neues Key-Material lokal **stagen** → `space-rotate` senden → **Bestätigung abwarten** → erst dann lokal committen + `key-rotation`/`member-update` verteilen (zweiphasig). Offline (Broker unerreichbar): Status `pending/not-enforced`, retrybare Outbox.
- **Sicherheitseigenschaft „removed ⟺ broker rotated":** Vor Enforcement ist der zu entfernende Member kanonisch noch Member — seine Writes sind legitim. Ab Enforcement greifen **gleichzeitig** die sofortige [Scope-Invalidierung](003-transport-und-broker.md#capability-widerruf-über-rotation) **und** das Broker-Ingest-Generations-Gate (Sync 003, `keyGeneration < space.generation → KEY_GENERATION_STALE`) — der entfernte Member kann danach **nichts** mehr in den durablen Log persistieren. Der un-gegatete Live-Kanal ist durch die [Relay-Whitelist](003-transport-und-broker.md#relay-whitelist-muss) geschlossen.
- **Offline-Admin = Liveness-Grenze, keine Safety-Verletzung:** Ohne erreichbaren Broker kann kein Enforcement stattfinden; der Member bleibt bis zur bestätigten Rotation Member. Festival-kritische Spaces (große, geteilte Spaces, in denen Moderation/Removal gebraucht wird) SOLLTEN daher **mehrere Admins** haben (siehe [Rollen](#rollen)), damit Removal nicht an einem einzelnen offline Admin hängt.
