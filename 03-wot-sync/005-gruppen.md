...
- **Austretender ist registrierter Admin (weitere Members verbleiben):** Er MUSS die [operationelle Reihenfolge](#operationelle-reihenfolge-muss) selbst ausfuehren (Staging → `space-rotate` → Commit + Verteilung), mit sich selbst als entferntem Member. Er erzeugt dabei zwingend das Folgegenerations-Key-Material; er DARF die `key-rotation` NICHT an sich selbst oder eigene Geraete zustellen und MUSS das neue Key-Material (Content Key + Capability-Keypair) nach abgeschlossener Verteilung unwiderruflich aus lokalem und durablem State loeschen — einschliesslich des Removal-Staging-States.
