...
3. **`exp`** prüfen — `exp` MUSS als JWT NumericDate gemäß RFC 7519 §2 als nicht-negativer ganzzahliger Unix-Sekunden-Wert kodiert sein. Fractional NumericDate-Werte, String- oder Boolean-Werte, negative Werte sowie ein fehlendes `exp`-Feld MÜSSEN abgelehnt werden. Verifier MÜSSEN die aktuelle Verifikationszeit als ganzzahlige Unix-Sekunden normalisieren, indem sie eine Millisekunden-genaue Eingangszeit mit `floor(now_in_milliseconds / 1000)` auf den UTC-Sekundenstrahl abbilden; Sub-Sekunden-Vergleiche DÜRFEN nicht für diesen Check verwendet werden. Die Trust-List MUSS abgelehnt werden, wenn `exp <= verificationTimeSeconds` gilt. `exp == verificationTimeSeconds` MUSS daher als abgelaufen abgelehnt werden; gültige `exp`-Werte MÜSSEN strikt nach der normalisierten Verifikationszeit liegen.
4. **`iat`** prüfen — `iat` MUSS als JWT NumericDate gemäß RFC 7519 §2 als nicht-negativer ganzzahliger Unix-Sekunden-Wert kodiert sein. Fractional NumericDate-Werte, String- oder Boolean-Werte, negative Werte sowie ein fehlendes `iat`-Feld MÜSSEN abgelehnt werden. Verifier MÜSSEN dieselbe `floor(now_in_milliseconds / 1000)`-Normalisierung wie für `exp` verwenden und die Trust-List ablehnen, wenn `iat > verificationTimeSeconds` gilt. `iat == verificationTimeSeconds` MUSS akzeptiert werden; die `iat`-Grenze ist inklusiv.
...
`exp` und `iat` sind die einzigen normativen Trust-List-NumericDate-Claims in `wot-hmc@0.1`. Diese Sektion definiert keine generische JWT-NumericDate-Behandlung außerhalb der HMC H01 Trust List und ersetzt insbesondere nicht die Trust-001-`nbf`/`exp`-Behandlung oder Device-Delegation-Zeitvergleiche.
