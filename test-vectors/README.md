...
Test-Vektoren fuer das JSON Canonicalization Scheme (RFC 8785). Verschiedene Implementierungen MÜSSEN fuer dieselben Eingaben bytengenau dieselbe Ausgabe erzeugen.
...
8. Fuer `wot-sync@0.1` MUSS eine Implementierung den `device-revoke`-Control-Frame mit `revocationJws` akzeptieren und malformed Frame-Formen ablehnen.
9. Fuer `wot-device-delegation@0.1` MUSS eine Implementierung die Device-Delegation-Vektoren in [`device-delegation.json`](device-delegation.json) verifizieren und Negativfaelle ablehnen.
...
- **SD-JWT VC** (HMC) - Trust-List-Signatur, Disclosure-Hashes, Selective Disclosure und HMC H01 Trust-List `exp`/`iat`-NumericDate-Semantik (nicht-negative ganzzahlige Unix-Sekunden, `floor(now_in_milliseconds/1000)`-Normalisierung der Verifikationszeit, inklusive `iat`-Grenze und exklusive `exp`-Grenze) inklusive Negativfaellen fuer fehlende, String-, Boolean-, fractional, negative, abgelaufene `exp` und zukuenftige `iat`. Der `vct`-Wert im Vektor ist beispielhaft (RFC 2606 `.example`-Domain); `wot-hmc@0.1` legt keinen festen Produktions-`vct` fest und Verifier MÜSSEN den erwarteten `vct` per Konfiguration erhalten (siehe [H01](../05-hmc-extensions/H01-trust-scores.md#sd-jwt-vc-validation-muss)).
